  ...
  <v-row no-gutters>
    ...
    <template v-if="bgSwitch">
      <v-col class="pa-2" cols="12" md="6">
        <v-radio-group
          v-model="options.cornersDotOptions.gradient.type"
          class="ma-0"
          row
      ...
      <v-col class="pa-2" cols="12" md="6">
        <color-picker
          v-model="options.cornersDotOptions.gradient.colorStops[0].color"
          :label="$t('first_color')"
          @input="$emit('update', true)"
        />
      </v-col>
      ...
      <v-col class="pa-2" cols="12" md="6">
        <color-picker
          v-model="options.cornersDotOptions.gradient.colorStops[1].color"
    ...
    <v-col v-else class="pa-2" cols="12" md="6">
      <color-picker
        v-model="options.cornersDotOptions.color"
        :label="$t('dot_colors')"
        @input="$emit('update', true)"
      />
    </v-col>
...
export default {
  ...
  data: () => ({
    bgSwitch: false,
    designObject: [
      { value: "square", img: "/media/eye/square.svg" },
      { value: "dot", img: "/media/eye/circle.svg" },
    ],
    rotations: 0,
  }),
  ...
  computed: {
    ...
    computedRotation: {
      ...
      set(val) {
        this.rotation = val;
        this.options.cornersDotOptions.gradient.rotation =
          (val * Math.PI) / 180;
  ...
  watch: {
    bgSwitch(val) {
      if (!val) this.options.cornersDotOptions.gradient = null;
      else {
  ...
  methods: {
    chooseDesign(value) {
      this.options.cornersDotOptions.type = value;
      this.$emit("update");
    },
  },
};

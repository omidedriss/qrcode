    ...
    <div class="content-wrapper">
      ...
      <div class="preview-area" ref="createButton">
        ...
        >
          ...
          <router-link
            :to="{
              path: selectedButton.link2 || '/create-qr-code',
              query: { type: selectedButton.text2 || selectedButton.text },
            }"
            custom
            v-slot="{ navigate }"
          >
        ...
        <div class="sheet-text">
          ...
          >
            <v-btn
              ...
              style="margin-top: 16px"
            >
              {{ $t("create_button", [$t(selectedButton.text2)]) }}
            </v-btn>
          </router-link>
        </div>
      </v-sheet>
    ...
    "settings.dynamicButtons": {
      handler(newButtons) {
        this.dynamicButtons = newButtons.map((button) => ({
          ...
          link1: button.link1 || "", // Add default for link1
          link2: button.link2 || "/create-qr-code", // Add default for link2
        }));
      },
      immediate: true,
      deep: true,
  ...
  methods: {
      ...
      if (this.$vuetify.breakpoint.smAndDown) {
        ...
        this.$nextTick(() => {
          ...
          if (image) {
            ...
            const targetY = rect.top + scrollTop + rect.height + 10;
            window.scrollTo({
              top: targetY,
              behavior: "smooth",
            });
    ...
    createAction() {
      if (this.selectedButton.link1) {
        window.location.href = this.selectedButton.link1; // External link
      } else {
        console.log(this.$t("create_business_card_clicked"));
      }
    },
    createAction2() {
      if (this.selectedButton.link2) {
        this.$router.push({
          ...
          query: {
            type: this.selectedButton.text2 || this.selectedButton.text,
          },
        });
      } else {
        this.$router.push({
...
[dir="rtl"] .btn-sheet .fa {
  background-image: linear-gradient(to top, #403199, #e9354d);
}
